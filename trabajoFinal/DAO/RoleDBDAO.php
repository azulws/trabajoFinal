<?php 
   ...
   protected function mapear($value) {
       ...
       foreach($value as $v){
           $role = new Role();
           $role->setRoleId($v['role_id']);
           $role->setRoleDescription($v['role_description']);
   ...
   public function Add( User $user){
       ...
       $parameters['role_id'] = $user->getRoleId();
       $parameters['role_description'] = $user->getRoleDescription();
   ...
   public function read ($role_id)
   {
       ...
       try
       {
           ...
           if(!empty($resultSet))
           {
               ...
               $role = new Role();
               $role->setRoleId($result[0]->getRoleId());
               $role->setRoleDescription($result[0]->getDescriptionRole());
               return $role;
           }else
               return false;
       }
